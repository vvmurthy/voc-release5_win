...
%BASE_DIR    = '/var/tmp/rbg'; need to change parent directory
BASE_DIR    = 'C:\Users\Veda\Documents\MATLAB\voc-release5';
...
% System version
conf = cv_test(conf, 'version', 'voc-release5');

% Project name (used in the paths)
conf = cv_test(conf, 'project', PROJECT);

% Parent directory that everything (model cache, VOCdevkit) is under
conf = cv_test(conf, 'paths.base_dir', BASE_DIR);

% Path to this file
conf = cv_test(conf, 'paths.self', fullfile(pwd(), [mfilename() '.m']));

% byte size of a single (should always be 4-bytes!)
%tmp = single(0);
%tmp = whos('tmp');
%tmp.bytes;
conf = cv_test(conf, 'single_byte_size', 4);
...
conf = cv_test(conf, 'pascal.year', PASCAL_YEAR);

% Directory with PASCAL VOC development kit and dataset
conf = cv_test(conf, 'pascal.dev_kit', [conf.paths.base_dir '/VOC' ...
...
conf = cv_test(conf, 'paths.model_dir', [conf.paths.base_dir '/' ...
                                    conf.project '/' conf.pascal.year '/']);

%This line gives the error "the syntax of the command is incorrect"                                
...
% -------------------------------------------------------------------
conf = cv_test(conf, 'training.train_set_fg', 'trainval');
conf = cv_test(conf, 'training.train_set_bg', 'train');
conf = cv_test(conf, 'training.C', 0.001);
conf = cv(conf, 'training.bias_feature', 10);
% File size limit for the feature vector cache (2^30 bytes = 1GB)
conf = cv_test(conf, 'training.cache_byte_limit', 3*2^30);
% Location of training log (matlab diary)
conf.training.log = @(x) sprintf([conf.paths.model_dir '%s.log'], x);

conf = cv_test(conf, 'training.cache_example_limit', 24000);
conf = cv_test(conf, 'training.num_negatives_small', 200);
conf = cv_test(conf, 'training.num_negatives_large', 2000);
conf = cv_test(conf, 'training.wlssvm_M', 0);
conf = cv_test(conf, 'training.fg_overlap', 0.7);

conf = cv_test(conf, 'training.lbfgs.options.verbose', 2);
conf = cv_test(conf, 'training.lbfgs.options.maxIter', 1000);
conf = cv_test(conf, 'training.lbfgs.options.optTol', 0.000001);

conf = cv_test(conf, 'training.interval_fg', 5);
conf = cv_test(conf, 'training.interval_bg', 4);


% -------------------------------------------------------------------
% Evaluation configuration 
% -------------------------------------------------------------------
conf = cv_test(conf, 'eval.interval', 10);
conf = cv_test(conf, 'eval.test_set', 'test');
conf = cv_test(conf, 'eval.max_thresh', -1.1);
...
% -------------------------------------------------------------------
conf = cv_test(conf, 'features.sbin', 8);
conf = cv_test(conf, 'features.dim', 32);
conf = cv_test(conf, 'features.truncation_dim', 32);
conf = cv_test(conf, 'features.extra_octave', false);


% -------------------------------------------------------------------
% Cascade configuration 
% -------------------------------------------------------------------
%conf = cv(conf, 'cascade.data_dir', [pwd() 'voc-release5\star-cascade\data\']);
conf = cv_test(conf, 'cascade.data_dir', 'C:\Users\Veda\Documents\MATLAB\voc-release5\star-cascade\data\');
...
% ------------------------------------------------------------------
%FUNCTION HAS BEEN FIXED TO WORK WITH WINDOWS UNDER THE ASSUMPTION YOU HAVE
%ALL THE DIRECTORIES SET
%----------------------------------------------------------------------
%function made = exists_or_mkdir(path)
function exists_or_mkdir(path)
%made = false;
if exist(path) == 0
  %this is a unix functionality and thus wont work in windows- it helps to just have all the paths set up already  
  %unix(['mkdir -p ' path]);
  %made = true;
end
