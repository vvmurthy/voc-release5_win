//cascade.cpp - converted to cpp so that the text has color to it.

#include "mex.h" //mex.h is MATLAB header to run c files
#include "model.h"
#include "wintimer.h" //name changed to wintimer due to naming errors
#include <vector> //Vector is in windows
#include <cmath> //cmath is in windows
#include <limits> //this is needed to define the value infinity that shows up occasionally in the script
                             ...
                             int pca, double defthresh) 
{
  ...
  for (int x = xstart; x <= xend; x++) {
    ...
    for (int y = ystart; y <= yend; y++) {
      ...
      double INFINITY = std::numeric_limits<double>::infinity(); //initializes the value infinity using the limits directory
      if (*ptr > -INFINITY)  //Undefined value? Infinity is not defined in any other code?
        continue;
